...
def summarize_simplified_data(resultsPath,last_seen_threshold=30,last_seen_min_y=300,threshold=1):
    ...
    for ntwk in glob.glob(resultsPath + '/*.csv'):
        ...
        ntwkDF['P_s2s'] = \
        ntwkDF['seen2seen']/s2s_denominator
...
#        ntwkDF.loc[s2s_denominator == 0,'P_s2s'] = 0 #was never seen, so P_s2s is 0
        ...
        ntwkDF['P_u2s'] = \
        ntwkDF['unseen2seen']/u2s_denominator
...
#            print(summaryDF.shape)
        ...
        for col in summaryDF.columns.get_level_values(0).unique():
            ...
            if na_filter == 0:
                #if pvisible is 1, unseen to seen probability is 1
#                summaryCol.loc[summaryCol['P_visible'] == 1,'P_u2s'] = 1.0
                summaryCol.loc[summaryCol['P_u2s'].isna(),'P_u2s'] = 1.0/(summaryCol.loc[summaryCol['P_u2s'].isna(),'unseen'] + 1)
                
                #where s2s probability is nan, make it 0
                summaryCol.loc[summaryCol['P_s2s'].isna(),'P_s2s'] = 0.0 #summaryCol.fillna(0)
                summaryCol = summaryCol.dropna()###if there are still na values, drop them
            elif na_filter == 'drop':
...
>>>>>>> 0e21637324c940395b3cf8092d2e3aec91017430
